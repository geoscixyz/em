...
where:
 ...
 - \\( \\hat{\\mathbf{n}} \\) is an outward normal unit vector perpendicular to :math:`da`
...
Therefore the above expression states that a change in magnetic flux, equal to 1 Weber per second, will induce an electromotive force of 1 Volt along a closed path.
Using the aforementionned expression, the Weber (:math:`Wb`) can be expressed as:
...
For a rock unit defined by conductivity :math:`\sigma`, Ohm's law (:math:`{\bf j} = \sigma {\bf e}`) implies that a current density :math:`{\bf j}` is also induced by the primary field.
...
From Eq. :eq:`electromotive_force_time`, and we know that the electromotive force is equivalent to the voltage being induced in the wire.
...
However, the voltage induced within the receiver coils will have both real (in-phase) and imaginary (out-of-phase) components.
