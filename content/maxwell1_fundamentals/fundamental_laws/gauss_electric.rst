...
.. math::
  \mathbf{F} = \frac{q_1q_2}{4\pi r^2}\hat{\mathbf{r}} \;,
  :label: Coulomb_q
where :math:`\mathbf{F}` is the force between the two charges :math:`q_1` and :math:`q_2`, :math:`r` is the distance between the charges and :math:`\hat{\mathbf{r}}` is a unit vector in the direction of the line separating the two charges.
...
.. math::
  \mathbf{e} = \frac{1}{4\pi\epsilon_0}\int_V \frac{\rho}{r^2}\mathrm{d}\hat{\mathbf{r}}
  :label: e_charge_den
...
.. math::
   \nabla \cdot \mathbf{e} = \frac{1}{4\pi\epsilon_0}\int_V \nabla \cdot\left(\frac{1}{r^2}\right)\rho\mathrm{d}\hat{\mathbf{r}} = \frac{\rho}{\epsilon_0}
   :label: Gauss_diff
...
.. math::
	\boldsymbol{\Phi} = \int_s \mathbf{e} \cdot d\mathcal{s}
	:label: e_flux
