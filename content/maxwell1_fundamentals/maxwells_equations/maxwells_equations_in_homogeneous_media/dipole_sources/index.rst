...
For the electrical current dipole, the source term :math:`\mathbf{J_e}` represents an electrical current density and has units A/m :math:`\!^2`.
For the magnetic dipole, the source results from a magnetization :math:`\mathbf{M}`.
However, the magetic source term :math:`\mathbf{J_m} = - i\omega \mu \mathbf{M}` is generally represented by a magnetic current density, where :math:`\mathbf{J_m}` has units V/m :math:`\!^2`.
...
.. math::
	...
	\nabla \times \mathbf{H} - \sigma \mathbf{E} - & i\omega \varepsilon \mathbf{E} = \pm \, \mathbf{J_e}
	\end{split}
...
.. math::
	\begin{split}
	\nabla \times \mathbf{e} \, + \, &\dfrac{\partial \big [ \mu \mathbf{h} \big ]}{\partial t} = \pm \mathbf{j_m} \\
	\nabla \times \mathbf{h} - \sigma & \mathbf{e}  - \dfrac{\partial \big [ \varepsilon \mathbf{e} \big ]}{\partial t} = \pm \mathbf{j_e}
	\end{split}

where :math:`\mathbf{j_e}` and :math:`\mathbf{j_m}` are time-dependent electric and magnetic source terms, respectively.
